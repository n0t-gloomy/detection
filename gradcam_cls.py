...
def _pick_target_layer(
    ...
    # Auto: last Conv2d that still has H,W > 1 on a probe forward
    probe = torch.zeros(1, 3, imgsz, imgsz, device=device)
    handles: list = []
    spatial_last: tuple[nn.Module, str] | None = None
    def make_hook(n: str):
        def hook(_m, _inp, out):
            nonlocal spatial_last
    ...
    try:
        ...
        for h in handles:
            h.remove()
    if spatial_last is not None:
        return spatial_last
    # Fallback: last conv in the model
...
class _GradCAM:
    ...
    def _backward_hook(self, _m, _gin, gout):
        g = gout[0]
        if g is not None:
